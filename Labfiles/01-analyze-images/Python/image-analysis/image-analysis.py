...
def AnalyzeImage(image_filename, image_data, cv_client):
    ...
    if result.objects is not None:
        ...
        outputfile = os.path.join(outputdir, f"objects_{os.path.basename(image_filename)}")
    ...
    if result.people is not None:
        ...
        plt.imshow(image)
        plt.tight_layout(pad=0)
        outputfile = os.path.join(outputdir, f"people_{os.path.basename(image_filename)}")
        fig.savefig(outputfile)
        print('  Results saved in', outputfile)
...
def BackgroundForeground(endpoint, key, image_file):
    ...
    outputfile = os.path.join(outputdir, f"{mode}_{os.path.basename(image_file)}")
